...
(cond  ((null? right) left)
      ((stream-empty? left) (stream-cons (first right) Acc))
      (else (stream-cons (stream-first left) (rotate (stream-rest left) (cdr right) (stream-cons (first right) Acc)))))) 
...
(define (enqueue x q)
(if(>= (queue-size-l q) (+ (queue-size-r q) 1))
(queue (queue-left q) (cons x (queue-right q)) (queue-size-l q) (+ (queue-size-r q) 1))
(queue (rotate (queue-left q) (cons x (queue-right q)) empty-stream) '() (+ (queue-size-l q) (+ (queue-size-r q) 1)) 0)) )
...
(define (dequeue q)
  (if(<= (queue-size-l q) (queue-size-r q) )(queue (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream) '() (+ (- (queue-size-l q) 1) (queue-size-r q)) 0)
     (queue (stream-rest (queue-left q)) (queue-right q) (- (queue-size-l q) 1) (queue-size-r q))))
...
(define (top q)
  (if(stream-empty? (queue-left q)) empty-stream
