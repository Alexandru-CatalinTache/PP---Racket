...
(define (queue-empty? q)
  (and (null? (queue-right q)) (null? (queue-left q))))
...
(define (enqueue x q)
  (queue (queue-left q) (reverse (append  (reverse (queue-right q)) (list x))) (queue-size-l q)  (+ (queue-size-r q) 1)))
...
; Veți întoarce coada actualizată.
(define (dequeue q)
  (if(null? (queue-left q)) (dequeue_empty_left q)
     (queue (cdr (queue-left q)) (queue-right q) (- (queue-size-l q) 1) (queue-size-r q))))

(define (dequeue_empty_left q)
  (dequeue (queue (reverse (queue-right q)) '() (queue-size-r q) 0)))
